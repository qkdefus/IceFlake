...
namespace IceFlake.Scripts
{
    ...
    public class DrawUnitsScript : Script
    {
        ...
        private void DrawCircle(Location loc, float radius, Color innerColor, Color outerColor, int complexity = 24, bool isFilled = true)
        {
            ...
            D3D.CurrDevice.DrawUserPrimitives(PrimitiveType.TriangleFan, buffer.Length - 2, buffer);
        }
        ...
        private void DrawLine(Vector3 from, Vector3 to, Color color)
        {
            ...
            D3D.CurrDevice.DrawUserPrimitives(PrimitiveType.LineStrip, vertices.Count - 1, buffer);
        }
        ...
        {
            if (cube == null)
                cube = Mesh.CreateBox(D3D.CurrDevice, width, height, depth);
        ...
        private void DrawText(Vector3 loc, string text)
        {
            ...
            var mesh = Mesh.CreateText(D3D.CurrDevice, font, text, 0f, 0f);
        ...
        {
            if (wireframe)
                D3D.CurrDevice.SetRenderState(RenderState.FillMode, FillMode.Wireframe);
            SetTarget(loc);
            mesh.DrawSubset(0);
            if (wireframe)
                D3D.CurrDevice.SetRenderState(RenderState.FillMode, FillMode.Solid);
        }
