...
namespace IceFlake.DirectX
{
    public static class Direct3D
    {
        ...
        private static int ResetHook(IntPtr device, ref SlimDX.Direct3D9.PresentParameters pp)
        {
            /*/
            [22:28:24] PresentParameters :
            [22:28:24] 	 77
            [22:28:24] 	 1
            [22:28:24] 	 22
            [22:28:24] 	 600
            ...
            /*/
            /*/
            Log.WriteLine(LogType.Critical, "PresentParameters :");
            Log.WriteLine(LogType.Critical, "\t " + pp.AutoDepthStencilFormat);
            Log.WriteLine(LogType.Critical, "\t " + pp.BackBufferCount);
            Log.WriteLine(LogType.Critical, "\t " + pp.BackBufferFormat);
            Log.WriteLine(LogType.Critical, "\t " + pp.BackBufferHeight);
            Log.WriteLine(LogType.Critical, "\t " + pp.BackBufferWidth);
            Log.WriteLine(LogType.Critical, "\t " + pp.EnableAutoDepthStencil);
            Log.WriteLine(LogType.Critical, "\t " + pp.Flags);
            Log.WriteLine(LogType.Critical, "\t " + pp.FullScreen_RefreshRateInHz);
            Log.WriteLine(LogType.Critical, "\t " + pp.hDeviceWindow);
            Log.WriteLine(LogType.Critical, "\t " + pp.MultiSampleQuality);
            Log.WriteLine(LogType.Critical, "\t " + pp.MultiSampleType);
            Log.WriteLine(LogType.Critical, "\t " + pp.PresentationInterval);
            Log.WriteLine(LogType.Critical, "\t " + pp.SwapEffect);
            Log.WriteLine(LogType.Critical, "\t " + pp.Windowed);
            Log.WriteLine(LogType.Critical, "\t --");
            /*/
            ...
            {
                AutoDepthStencilFormat = (Format)pp.AutoDepthStencilFormat,
                BackBufferCount = pp.BackBufferCount,
                BackBufferFormat = (Format)pp.BackBufferFormat,
                BackBufferHeight = pp.BackBufferHeight,
                BackBufferWidth = pp.BackBufferWidth,
                DeviceWindowHandle = pp.DeviceWindowHandle,
                EnableAutoDepthStencil = pp.EnableAutoDepthStencil,
                FullScreenRefreshRateInHertz = pp.FullScreenRefreshRateInHertz,
                Multisample = (MultisampleType)pp.Multisample,
                MultisampleQuality = pp.MultisampleQuality,
                PresentationInterval = (PresentInterval)pp.PresentationInterval,
                PresentFlags = (PresentFlags)pp.PresentFlags,
                SwapEffect = (SwapEffect)pp.SwapEffect,
            ...
            return (int)_resetHook.CallOriginal(device);
            //return (int)_endSceneHook.CallOriginal(CurrDevice);
        }
